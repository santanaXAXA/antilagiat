...
BORROW_PHRASES = [
    # Ссылки на источники
    "согласно", "по мнению", "как отмечает", "как указывает", "как пишет",
    "по данным", "согласно исследованиям", "как считает", "как полагает",
    "по словам", "как утверждает", "по результатам", "в работе",
    # Академические клише
    "ряд авторов", "многие учёные", "в научной литературе", "исследователи отмечают",
    "принято считать", "общепризнанным является", "классическое определение",
    "термин был введён", "впервые описан", "широко известно", "давно установлено",
    "является общепринятым", "в соответствии с теорией", "данная концепция",
    "согласно классификации", "в отечественной литературе", "зарубежные исследователи",
    # Пассивный залог (признак копипасты из учебников)
    "было показано", "было установлено", "было выявлено", "было определено",
    "рассматривается как", "определяется как", "понимается как", "трактуется как",
    "относится к", "является одним из", "представляет собой",
    # Типичные вводные для заимствований
    "следует отметить, что", "необходимо подчеркнуть, что", "известно, что",
    "установлено, что", "доказано, что", "показано, что",
]
...
def detect_borrowing(text: str) -> dict:
    ...
    # Длинные предложения (порог снижен: >20 слов уже подозрительно)
    sentences = split_sentences(text)
    long_sents = [s for s in sentences if len(s.split()) > 20]
    long_ratio = len(long_sents) / len(sentences) if sentences else 0

    # Пассивные конструкции (дополнительный признак)
    passive = len(re.findall(r'\b(был[аои]?|были|является|являются|считается|считаются)\b', lower))
    passive_ratio = passive / (len(words) / 100) if words else 0
